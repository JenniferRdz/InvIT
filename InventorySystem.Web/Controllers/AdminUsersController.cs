...
public class AdminUsersController : Controller
{
    ...
    public async Task<IActionResult> Create(AppUser model, string? password)
    {
        ...
        if (!string.IsNullOrWhiteSpace(password))
        {
            if (!PasswordPolicy.IsStrong(password, out var err))
                ModelState.AddModelError(nameof(password), err);
        }

        if (!ModelState.IsValid) return View(model);

        // Si no pasas password, generamos una temporal y forzamos cambio
        string shownPassword = password!;
        if (string.IsNullOrWhiteSpace(password))
        {
        ...
        TempData["ok"] = $"Usuario creado. Contraseña temporal: {shownPassword}";
    ...
    // Utilidad (temporal fuerte y fácil de dictar)
